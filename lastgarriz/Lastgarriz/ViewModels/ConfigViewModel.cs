                ...
                if (item.Keycode > 0 && item.Value?.Length > 0)
                {
                    switch (item.Fonction)
                    {
                        ...
                        case Strings.Feature.ARTILLERY_RU:
                            ...
                            break;
                        case Strings.Feature.ARTILLERY_UK:
                            Features.Artillery_uk.Hotkey = HotKey.GetModString(item.Modifier) + kc.ConvertToString(item.Keycode);
                            Features.Artillery_uk.IsEnable = item.Enable;
                            break;
                ...
                if (item.Keycode > 0 && item.Value?.Length > 0)
                {
                    switch (item.Fonction)
                    {
                        ...
                        case Strings.Feature.ARTILLERY_RU:
                            ...
                            break;
                        case Strings.Feature.ARTILLERY_UK:
                            item.Modifier = HotKey.GetModifier(Features.Artillery_uk.Hotkey);
                            item.Keycode = VerifyKeycode(Features.Artillery_uk, item.Keycode);
                            item.Enable = Features.Artillery_uk.IsEnable;
                            break;
                        case Strings.Feature.ARTILLERY_VALIDATE:
