            ...
            if (!Global.HotkeyProcBlock && message.Msg == NativeWin.WM_HOTKEY)
            {
                ...
                if (shortcut != null && shortcut.Value != null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (fonctionLower is Strings.Feature.ARTILLERY_USGER or Strings.Feature.ARTILLERY_RU or Strings.Feature.ARTILLERY_UK)
                            {
                                int metricsType = fonctionLower switch
                                {
                                    Strings.Feature.ARTILLERY_RU => 1,
                                    Strings.Feature.ARTILLERY_UK => 2,
                                    _ => 0
                                };
                                ...
                                if (pHwnd.ToInt32() > 0)
                                {
                                    ...
                                    TaskBar.ViewModel.ArtilleryUssr = TaskBar.ViewModel.ArtilleryUsGer = TaskBar.ViewModel.ArtilleryUk = false;
                                }
                                else
                                {
                                    Main?.Close(); // close mainWindow
                                    ArtilleryWindow artiWin = new(metricsType)
                                    {
                                        Name = Strings.View.ARTILLERY
                                    };
                                    artiWin.Show();
                                    artiWin.Visibility = Visibility.Visible;
                                    TaskBar.ViewModel.ArtilleryUssr = metricsType == 1;
                                    TaskBar.ViewModel.ArtilleryUsGer = metricsType == 0;
                                    TaskBar.ViewModel.ArtilleryUk = metricsType == 2;
                                    if (hllLaunched)
                                    {
